...
def get_argparser():
    ...
    parser.add_argument("--in_choices_file",
                        type=str,
                        help="File path of a list of the chosen ms/ms for each precursor",
                        required=True
                        )
...
if __name__ == "__main__":
    ...
    current_ref = -1
    with open(args.in_choices_file, "r") as infile:
        for line in tqdm(infile, "Precursor", miniters=1000):
            ...
            if ref != current_ref:
                current_ref = ref
                precursors_tsv = pd.read_csv(os.path.join(data_dir, f"{current_ref.zfill(4)}.tsv"), sep='\t')
                precursors_tsv = precursors_tsv[["Sequence", "Precursor", "Charge", "Matches", "Intensities", "id"]]
            
            chosen_row = precursors_tsv.query("`id` == @id").iloc[0]
            modified_peptide_seq = chosen_row["Precursor"][:-1]
            ...
            precursor_mz = mass.fast_mass2(transformed_pept_seq, aa_mass=aa_mass, ion_type="M", charge=chosen_row["Charge"])

            fragment_types, intensities = process_fragment_and_intensities_strings(chosen_row["Matches"], chosen_row["Intensities"])
            ...
            for annotation, intensity in zip(fragment_types, normalized_intensities):
                ...
                rows.append([precursor_mz,
                             ...
                             annotation, 
                             chosen_row['Sequence'], 
                             chosen_row['Precursor'][:-1], # remove charge
                             chosen_row['Charge'],
                             intensity,
                             0, #RT
                             0, #IM
